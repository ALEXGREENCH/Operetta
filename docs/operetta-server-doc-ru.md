...
| `u` | URL/путь (`/obml/<hint>/<url>`); `hint=0/1` — служебное состояние цепочки старого клиента, а не номер страницы |
...
| `w` | Legacy-состояние навигации; явная пагинация передаётся через `__om=page=N&pp=M` |
...
| Переменная | Описание |
|-------------|----------|
| `PORT` | Порт для `cmd/operetta` (по умолчанию `127.0.0.1:8081`; публичный bind задаётся явно флагом `-addr :8081`). |
...
| `OMS_ENABLE_DIAGNOSTICS` | `1` — включить сетевой `/validate`; по умолчанию endpoint отключён. |
